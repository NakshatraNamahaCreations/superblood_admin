  ...
  const fetchAllenquiryData = async () => {
    ...
    try {
      const response = await axios.get(
        "https://api.nakshatranamahacreations.com/api/getenquiry"
      );
  ...
  const handleConfirmStatusUpdate = async () => {
    try {
      const response = await axios.put(
        `https://api.nakshatranamahacreations.com/api/update-enquiry-status/${selectedRow._id}`
      );
      if (response.status === 200) {
        alert("Status updated successfully");
        setShowModal(false);
        fetchAllenquiryData(); // refresh table
      }
    } catch (error) {
  ...
  return (
    <div className="container">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Footer>
          <Button variant="btn btn-success" onClick={handleConfirmStatusUpdate}>
            Confirm
          </Button>
